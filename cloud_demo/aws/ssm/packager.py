...
class SSMPackageUpdater:
    ...
    def update(self, package_name, s3bucket, file_to_upload):
        """
        :param region:
...
class SSMPackageBuilder:
    def build(self, build_dir):
        self.build_dir = build_dir
        dirs = set()
        files = set()

        installer_list = self._parse_mappings(os.path.join(build_dir, 'agent_list.json'))
    ...
    def _generate_manifest(self, installer_list, hashes):
        ...
        with open(os.path.join(self.build_dir, 'manifest.json'), 'w') as file:
